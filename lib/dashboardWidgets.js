...
// for all of them, editable title/description only for the ones with
// real copy to override: the resource boxes, and (Update 55) the
// Resume review section's header — though unlike the resource boxes,
// resume_review's actual body content (the upload card, score, tips)
// is a real feature, not static copy, so only its heading text is
// overridable here, not "the entire content." A StatCard's "title" is
// really its data label (e.g. "New jobs today") tightly coupled to
// the number it's showing, so letting that drift from what it
// actually represents would be confusing rather than useful — those
// get a visibility toggle only.
//
...
export const WIDGET_DEFS = [
  ...
  {
    key: 'resume_review',
    label: 'Resume review (full-width section)',
    editableCopy: true,
    defaultTitle: 'Resume review',
    defaultDescription:
