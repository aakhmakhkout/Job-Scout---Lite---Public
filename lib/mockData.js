...
// Step 18: 3-tier system, replacing the old 4-tier one (Trusted/Good/
// Review/Suspicious). Suspicious is now reserved for jobs where a real
// negative signal actually fired — see scraper/trust_scoring.py's
// module docstring for the full reasoning.
//
// Accepts EITHER a raw number (a trust_score, for backward
// compatibility / places that only have the number) OR a job-like
// object with a trust_tier field. When trust_tier is present, it's
// used directly — it was computed at scrape/insert time with more
// information (whether a negative signal fired) than a component
// re-deriving a tier from trust_score alone could ever have. The plain
// score-only path below is a reasonable fallback, not the source of
// truth for admin/scraper-produced jobs.
...
function resolveTierKey(input) {
  if (typeof input === 'number') return tierKeyFromScore(input);
  if (input && typeof input === 'object') {
    if (input.trust_tier) return input.trust_tier.toLowerCase();
    return tierKeyFromScore(input.trust_score ?? 0);
  }
  return 'unverified';
}

export function trustBadgeLabel(input) {
  const key = resolveTierKey(input);
  if (key === 'trusted') return 'Trusted';
  if (key === 'suspicious') return 'Suspicious';
  return 'Unverified';
}

export function trustBadgeColor(input) {
  return resolveTierKey(input);
}
