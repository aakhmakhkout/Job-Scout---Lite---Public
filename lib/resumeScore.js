...
// Returns { score: 0-100, label, tips: [string, ...], breakdown: [...] }
// — tips are ordered roughly by impact: missing contact info and core
// sections first (the things most likely to get a resume auto-
// rejected or skipped), formatting/structure next, then section
// order, bullets, and word count last (real, but lower-stakes
// polish). `breakdown` groups the same underlying weighted checks
// into 4 display categories (earned/possible points each) — added in
// Update 66 specifically to give the Dashboard's compact box (and
// /profile's) something more concrete than a bare score to show;
// grouping is a presentation choice, the math underneath is unchanged
// from Update 52.
export function scoreResume(atsChecks) {
  ...
  const categories = {
    contact: { label: 'Contact & sections', earned: 0, possible: 0 },
    formatting: { label: 'Formatting & structure', earned: 0, possible: 0 },
    bullets: { label: 'Bullets & wording', earned: 0, possible: 0 },
    length: { label: 'Length', earned: 0, possible: 0 },
  };

  // Contact info — highest priority; a reviewer or an ATS that can't
  // find a way to reach you makes everything else moot.
  categories.contact.possible += WEIGHTS.hasEmail;
  if (contactInfo.hasEmail) categories.contact.earned += WEIGHTS.hasEmail;
  ...
  categories.contact.possible += WEIGHTS.hasPhone;
  if (contactInfo.hasPhone) categories.contact.earned += WEIGHTS.hasPhone;
  ...
  categories.contact.possible += WEIGHTS.hasExperience;
  if (sections.hasExperience) categories.contact.earned += WEIGHTS.hasExperience;
  else tips.push('No Experience section found — this is one of the most important sections on a resume.');

  categories.contact.possible += WEIGHTS.hasSkills;
  if (sections.hasSkills) categories.contact.earned += WEIGHTS.hasSkills;
  else tips.push('Add a dedicated Skills section — many ATS systems specifically scan for one.');

  categories.contact.possible += WEIGHTS.hasEducation;
  if (sections.hasEducation) categories.contact.earned += WEIGHTS.hasEducation;
  else tips.push('No Education section found.');

  categories.contact.possible += WEIGHTS.hasSummary;
  if (sections.hasSummary) categories.contact.earned += WEIGHTS.hasSummary;
  ...
  if (structure.checked) {
    categories.formatting.possible += WEIGHTS.structure;
    ...
    if (structureIssues.length === 0) {
      ...
      categories.formatting.earned += WEIGHTS.structure;
  ...
  if (sectionOrder && sectionOrder.matchesRecommended !== null) {
    categories.formatting.possible += WEIGHTS.sectionOrder;
    if (sectionOrder.matchesRecommended) {
      categories.formatting.earned += WEIGHTS.sectionOrder;
    } else {
      tips.push(
  ...
  categories.bullets.possible += WEIGHTS.bullets;
  if (bullets.total === 0) {
    ...
    categories.bullets.earned += Math.round(WEIGHTS.bullets * ratio);
    if (ratio < 0.6) {
      tips.push(
  ...
  categories.length.possible += WEIGHTS.wordCount;
  if (wordCount.status === 'ok') {
    categories.length.earned += WEIGHTS.wordCount;
  ...
  // Only categories that actually had something to score (formatting
  // can be 0/0 for a PDF with too few labeled sections to check order
  // and no structure checks run at all) — an empty "0/0 Formatting &
  // structure" row would look like a failing score rather than "not
  // applicable," so it's left out instead.
  const breakdown = Object.values(categories).filter((c) => c.possible > 0);
...
// Update 65 — moved here from components/account/ResumeUploadCard.jsx
// so both the /profile card and the Dashboard's compact
// ResumeSummaryBox can use the same "ATS-friendly" quick yes/no
// without duplicating the logic. Not a second, separately-computed
// score — just a plain-language read of whether the core ATS-relevant
// checks (contact info detectable, key sections present, structure
// checks when they were run at all) came back clean.
export function isAtsFriendly(checks) {
